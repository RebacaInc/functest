...
class AbotEpc(vnf.VnfOnBoarding):
    ...
    def __init__(self, **kwargs):
        if "case_name" not in kwargs:
            kwargs["case_name"] = "abot_epc"
        super(AbotEpc, self).__init__(**kwargs)
...
epc_credential.yaml".format(os_utils.get_credentials()['user_domain_name'],
                            self.case_dir)
            os.system(cmd)
...
abot_epc_credential.yaml".format(os_utils.get_credentials()\
    ...
    def deploy_orchestrator(self):
        ...
        external_router = CONST.__getattribute__(
        ...
        self.__logger.info("Creating External Router ....")
        ...
        self.__logger.info("JUJU Bootstraping ....")
        os.system('juju bootstrap abot-epc abot-controller --config \
                   network={} --metadata-source ~  --constraints mem=1G \
    ...
    def deploy_vnf(self):
        ...
        download_url(descriptor.get('url'), self.case_dir)
        ...
        os.system('juju deploy {}'.format(self.case_dir + '/' + 
        ...
        if status == 0:
            ...
            os.system('juju run-action abot-epc-basic/0 run \
                       tagnames={}'.format('Attach_Procedure_AttachWithIMSI'))
            os.system('juju-wait')
            return True
        else:
            return False
    ...
    def test_vnf(self):
        ...
        os.system('juju scp abot-epc-basic/0:/var/lib/juju/agents/\
unit-abot-epc-basic-0/charm/lib/artifacts/TestResults.json {}/.'.\
                   format(self.case_dir))
